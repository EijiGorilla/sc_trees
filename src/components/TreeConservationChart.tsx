...
///*** Others */
/// Draw chart
const TreeConservationChart = (props: any) => {
  // 1. Land Acquisition
  ...
  useEffect(() => {
    generateTreeConservationData().then((result: any) => {
      setTreesData(result);
    });
    ...
    zoomToLayer(treeConservationLayer);
  }, [props.contractp]);
  ...
  useEffect(() => {
    ...
    pieSeries.slices.template.events.on('click', (ev) => {
      ...
      view.when(() => {
        view.whenLayerView(treeConservationLayer).then((layerView): any => {
          //chartLayerView = layerView;

          treeConservationLayer.queryFeatures(query).then((results: any) => {
            ...
            treeConservationLayer.queryExtent(queryExt).then((result: any) => {
              if (result.extent) {
